...
function cors(data, statusCodeOrOptions = 200) {
  const {
    ...
    statusCode = 200
  } = src_default.number(statusCodeOrOptions) ? { statusCode: statusCodeOrOptions } : statusCodeOrOptions;
...
async function request(url, options = {}) {
  ...
  if (body) {
    params.body = src_default.plainObject(body) ? JSON.stringify(body) : body;
  }
...
function isValidDate(input) {
  const date = src_default.date(input) ? input : new Date(input);
...
function conditional(cases, defaultCase) {
  ...
  return defaultCase == null ? void 0 : defaultCase();
}
...
function invariant(condition, message) {
  ...
  const value = src_default.function(message) ? message() : message;
...
function px(value) {
  return src_default.number(value) || src_default.numericString(value) ? `${value}px` : value;
}
...
function getNestedProperty(input, path) {
  if (!src_default.plainObject(input) && !src_default.array(input) || !path) {
    return input;
  }
  ...
  for (let index = 0; index < length; index++) {
    ...
    if (currentSegment === "+" && src_default.array(output) && remainingSegments.length === 1) {
...
function queryStringFormat(input, options = {}) {
  ...
  const isValidInput = Object.values(input).every((item) => {
    if (src_default.array(item)) {
      return item.every((d) => src_default.string(d) || src_default.number(d));
    }
    return src_default.string(item);
  });
  if (!isValidInput) {
  ...
  const output = Object.entries(input).map(([key, value]) => {
    ...
    const nextValue = src_default.array(value) ? value.map(encoder).join(",") : encoder(`${value}`);
...
function mean(input, precision) {
  ...
  if (src_default.number(precision)) {
...
//# sourceMappingURL=chunk-7PF3YPDF.js.map
